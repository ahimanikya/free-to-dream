...
Local media is deliberately ignored by Git. A fresh clone has the text, images and catalog, but does not include these local recordings. Approved, hosted recordings appear once their public URLs are configured. The original Suno link is also available on the Odia page.
...
| `catalog/asset-inventory.json` | Checksums and filenames of imported local assets |
...
`build` without `--local-media` makes a public-safe site: it never copies local recordings, reference photographs or publication evidence. Only recording entries explicitly approved for release are embedded remotely. Cover artwork and the language drafts remain part of the public build.
